...
from coyomap.changelog import apply, check, lint, load_log
...
def test_the_update_chain_on_the_real_mcpolis_report():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # The report's own sentences are long; the readability check says so, as advice.
        assert all(w.startswith("entry ") for w in p.warnings), p.warnings
        ...
        assert all(w.startswith("the code touched ") for w in g.warnings), g.warnings
