...
def iter_prose_fields(model: ProjectModel, *, wide: bool = True) -> Iterator[tuple[str, str]]:
    ...
    # ONE ENTRY FOR THE PAIR, although the map holds two fields. A BOX is the unit these checks read
    # by, and the box a reader meets is the card, which shows the trigger and the outcome together.
    # Walking them apart was tried and measured: 29 findings appeared across the six live maps, every
    # one of them a "bare pointer", and every one false — an outcome opening "They" or "That
    # teammate" points at the person the trigger just named, two words away on the same card. Nought
    # true findings, twenty-nine false ones.
    #
    # Joined with a SPACE, never with the card's arrow: the sentence split reads a full stop, so an
    # arrow in the middle would fuse two sentences into one and every pair would trip the word limit.
    for uc in model.use_cases:
        yield f"{uc.id} trigger and outcome", f"{uc.trigger} {uc.outcome}".strip()
